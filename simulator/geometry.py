...
class City:
    ...
    def __init__(self, **config):
        ...
        # Use Chicago dataset instead of random requests?
        try:
            self.use_chicago = config['use_chicago']
            self.districts_coords = load(open('data/coordinates.json'))[str(config['chicago_grid_size'])]

            # Move squares lower left edge coordinates to square's center
            # Subtract latitudes of first two coords
            square_size = round(self.districts_coords[1][1] - self.districts_coords[0][1], 3)
            self.centered_districts_coords = [
                [coord[0] + square_size / 2, coord[1] + square_size / 2] for coord in self.districts_coords
            ]
        except KeyError:
            self.use_chicago = False

        # grid dimensions
        if not self.use_chicago:
            self.n = config["n"]  # number of pixels in x direction
            self.m = config["m"]  # number of pixels in y direction
        else:
            self.chicago = Chicago('data/test_trips.csv',
                                   config["start_date"],
                                   config["end_date"],
                                   (time(config["working_hrs_1"][0]), time(config["working_hrs_1"][1])),
                                   (time(config["working_hrs_2"][0]), time(config["working_hrs_2"][1])))
            self.n = int(self.chicago.n)
            self.m = int(self.chicago.m)
        ...
        self.bfs_trees = {}
        print("Creating BFS trees...\n")
        for c in tqdm(range(self.n*self.m), mininterval=1):
    ...
    def create_one_request_coord(self, sim_time):

        if self.use_chicago:
            ox, oy, dx, dy = self.chicago.get_one_request_coord(sim_time)
