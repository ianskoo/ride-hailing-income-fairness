...
class Simulation:
    ...
    def __init__(self, **config):
        ...
        if "max_time" in config:
            ...
            self.progress_bar_time = self.max_time // 70
        ...
        # Chicago ML prediction
        if self.behaviour == "ml_dispatcher_v2" or self.behaviour == "ml_dispatcher_distributor":
            self.city.chicago.init_prediction(config["ml_model_path"])
            self.ml_weighted = config["ml_weighted"]
    ...
    def add_taxi(self):
        ...
        if self.city.use_chicago:
            home = self.city.chicago.create_chicago_taxi_home_coords()
        else:
    ...
    def add_request(self):
        ...
        ox, oy, dx, dy = self.city.create_one_request_coord(self.time)  # Chris: added time for real-world data
    ...
    def run_batch(self, run_id, data_path='results'):
        ...
        d = round(self.num_taxis / ((self.city.n/10) * (self.city.m/10)), 2)
        print(f"Driver density d: {d} (taxis/km2)")
        ...
        for i in range(self.num_iter):
            # tick the clock
            for k in tqdm(range(self.batch_size), mininterval=2):
                self.step_time("")
    ...
    def step_time(self, handler):
            ...
            for taxi_id in self.taxis:
                ...
                # if a taxi can drop off its passenger, do it
                    ...
                    if (t.x == r.dx) and (t.y == r.dy):
                        ...
                        # ML-based dispatcher (requires Chicago data)
                        elif self.behaviour == "ml_dispatcher_v2" and self.city.use_chicago:
                            self.ml_dispatcher_poorest(taxi_id, self.ml_weighted)

                        elif self.behaviour == "ml_dispatcher_distributor" and self.city.use_chicago:
                            self.ml_dispatcher_weighted_distributor(taxi_id)
        ...
        # If using Chicago dataset, set request_rate dynamically based on number of requests in data
        if self.city.use_chicago:
            self.request_rate = self.city.chicago.get_request_rate(self.time)
            # print(f"time unit: {self.time}, request rate: {self.request_rate}")
        ...
        # step time and print progress
        self.time += 1
        # if not self.time % self.progress_bar_time:
        #     num_bars = int(self.time / self.progress_bar_time)
        #     print(f"[{'|' * num_bars}{' ' * (70 - num_bars)}]")
    ...
    def ml_dispatcher_poorest(self, curr_taxi_id, weighted: bool):
        ...
        # TODO: ev. check if it is worth to dispatch drivers by relative or absolute demand threshold
    ...
    def ml_dispatcher_weighted_distributor(self, curr_taxi_id: int):
        """Redistribute drivers to city areas based on predicted future demand, independently of their current income.
        Dispatch a driver having curr_taxi_id to an area with a probability based on predicted demand in 30'. """
        # Get predicted demand
        predictions = self.city.chicago.predict_demand(self.time)
        if predictions is None:
            return None
        ...
        dispatch_district_idx = np.where(predictions == np.random.choice(predictions, p=dispatch_weights))[0][0]
